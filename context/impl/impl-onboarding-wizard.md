...
| T-006 | DONE | preflight.ts (10 checks, DI) + preflight.test.ts (13 cases covering every surviving AC + secret-leak regression). 2026-04-20: R6 AC10 ("backend reachable") removed — the wizard now runs standalone, no HTTP round-trip. Moved to cavekit-onboarding-backend.md R4 (shared-token pickup + startup canary) and implemented in security._resolve_bot_shared_token + bot_token_source + gui startup log. |
