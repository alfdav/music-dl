...
- `Cargo.toml` must be generated from or mechanically rewritten from the authoritative Tauri app version during release prep, or the build must fail
...
v1 intentionally supports only these macOS install locations for auto-update:
- `/Applications/music-dl.app`
- `~/Applications/music-dl.app`
...
If the app is running from an unsupported install context, automatic update checks should no-op and the Settings action should explain that updates require the installed app in Applications.
...
6. if the sidecar is still alive after that timeout, abort install, return to an error state, and keep the current app session running rather than racing install against a live child process
