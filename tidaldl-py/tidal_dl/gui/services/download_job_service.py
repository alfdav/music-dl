...
def _download_dependencies() -> tuple[Any, Any, Any]:
    global Settings, Tidal, Download, DownloadOutcome
...
class DownloadJobService:
    ...
    def _execute_download_job(self, job: DownloadJob) -> None:
        ...
        outcome_value = None
        if isinstance(outcome_result, tuple) and len(outcome_result) == 2:
            outcome_value, _outcome_path = outcome_result
        if DownloadOutcome is not None and outcome_value == DownloadOutcome.FAILED:
            raise RuntimeError(
                f"Download failed for '{name}' (track {job.track_id}): stream fetch or "
                "TIDAL authorization failed. Check TIDAL login/token status."
            )
        if self._is_cancel_requested(current):
            self._mark_cancelled(current)
            return
