...
Library cache rows are not deleted until a scan traversal succeeds. Do not clear, age, or prune `scanned` rows at scan start. Mark-and-sweep skipped trash paths (`#recycle`, `.Trash`, and the shared skip list in `library_scanner.py`) and missing files only after a complete walk. Leftover skipped-directory rows are also dropped on first library/home DB open and never returned from library/albums/search/home, so a fingerprint-skip or a skipped Sync cannot keep serving `/Music/#recycle/...` (NAS recycle/trash on UGreen, Synology, and any NAS that uses that path component). A `#recycle` path component is never an artist — untagged files do not take that folder name. Unified search must not rank those paths first. An album that is mostly leftover recycle files is not Various Artists. An interrupted or failed scan must leave the previous good cache intact. Stage metadata outside a writer transaction and commit short batches; never hold a writer lock across filesystem walks or tag reads. Scan status must expose a named `phase` and increment `scanned` during discovery so the UI cannot sit on `scanned:0,total:0` with no explanation. Do not mutagen already-tagged rows that already have a real artist/title/album — a leftover `metadata_complete=0` from a schema migration is not a reason to re-read thousands of NAS files. Discover/walk first; leftover repair is a cheap DB filter plus placeholder rows only. Never open skipped-directory paths for repair.
