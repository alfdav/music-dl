...
Selected audio quality is a ceiling. Lossy `LOW`/`HIGH` stay exact. Lossless settings accept FLAC `LOSSLESS`/`HI_RES`/`HI_RES_LOSSLESS` when that is all Tidal has. Catalog `audioQuality` is often `LOSSLESS` even when `mediaMetadata.tags` include `HIRES_LOSSLESS`. OAuth `playbackinfopostpaywall` (Tidal Web) silently returns CD 16/44.1 for that request; OpenAPI `trackManifests` `formats` is the capability check (`FLAC_HIRES` when the account/track has Hi-Res). A Hi-Res setting then selects an unencrypted Hi-Fi Hi-Res stream, picking the `FLAC_HIRES` DASH representation rather than the first CD rep. If Tidal does not offer `FLAC_HIRES`, the CD file is accepted. If Tidal offers `FLAC_HIRES` but no unencrypted Hi-Res stream is available, the download fails instead of writing the CD file. Login quality probing uses `highestSoundQuality` (still `HI_RES` on Max accounts) and never changes or saves your selected quality.
