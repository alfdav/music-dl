...
- Tier 5 T-013 + T-014. dispatch_wizard (bun → node --import tsx fallback, inherited stdio, never aborts server), run_first_run_flow (R4+R5 orchestrator), --setup-bot Typer flag wired into gui command. 16 new backend tests (dispatch / skip / never / wizard-failure / force-bypass / force-non-modification / force-non-TTY). 44/44 backend. All 14 tasks DONE.
