...
class BrowseMixin:
    def artists_page(
        ...
        assert self._conn
        where = f"status != 'unreadable' AND artist IS NOT NULL AND {visible_scanned_path_sql()}"
        params: list = []
        if query:
    ...
    def all_albums(self, query: str = "") -> list[dict]:
        ...
        assert self._conn
        where = f"album IS NOT NULL AND status != 'unreadable' AND {visible_scanned_path_sql()}"
        params: list = []
        if query:
