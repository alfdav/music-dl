...
def bot_command(
    token: Annotated[
        ...
        typer.Option(
            ...
            help="Discord bot token. Falls back to DISCORD_BOT_TOKEN env var or macOS Keychain.",
        ),
    ] = None,
    channels: Annotated[
        list[int] | None,
    ...
    Paste Tidal URLs in Discord or use /dl, /queue, /history.
    """
    resolved_token = token
    if not resolved_token:
        resolved_token = _resolve_bot_token()
    ...
    if not resolved_token:
        Console().print(
            ...
            "  macOS Keychain: security add-generic-password -a music-dl -s DISCORD_BOT_TOKEN -w <token>",
        )
        raise typer.Exit(code=1)

    try:
        from tidal_dl.discord_bot import run_bot
    except ImportError:
        Console().print(
            "[red]discord.py is not installed.[/red]\n"
            "Install it with: uv pip install 'music-dl[discord]'",
        )
        raise typer.Exit(code=1)
...
def _resolve_bot_token() -> str | None:
    """Try macOS Keychain for the bot token."""
    import subprocess

    try:
        result = subprocess.run(
            ["security", "find-generic-password", "-a", "music-dl", "-s", "DISCORD_BOT_TOKEN", "-w"],
            capture_output=True,
            text=True,
            timeout=5,
        )
