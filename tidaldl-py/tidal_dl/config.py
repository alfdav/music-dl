...
class Tidal(BaseConfig[ModelToken]):
            ...
            try:
                ...
                try:
                    ...
                    if isinstance(_raw_exp, datetime):
                        ...
                        expiry_time = datetime.utcfromtimestamp(float(_raw_exp))
                    else:
                        expiry_time = None
    ...
    def token_persist(self) -> None:
        ...
        if hasattr(_exp, "timestamp"):
            ...
            _exp_value = _exp.timestamp()
        else:
            _exp_value = _exp
        self.set_option("expiry_time", _exp_value)
        self.save()
    ...
    def _ensure_token_fresh(self, refresh_window_sec: int = 300) -> bool:
        ...
        if hasattr(_raw_exp, "timestamp"):
            if getattr(_raw_exp, "tzinfo", None) is None:
                _raw_exp = _raw_exp.replace(tzinfo=timezone.utc)
            expiry_time = _raw_exp.timestamp()
        else:
